...
def reconstruct(df_seq, tree, id_col='id', sequence_col='sequence',
                working_dir='', save_ancestors=False, altall_cutoff=0.2,
                infer_gaps=True, aaRatefile='lg', **kwargs):
    """Use PAML to recontruct ancestral sequences.

    Parameters
    ----------
    df_seq : phylopandas.DataFrame
        dataframe containing information (including sequences) about tips of
        tree.
    ...
    altall_cutoff : float (default=0.2)
        probability cutoff for selecting alternative sites to flip in AltAll
        sequence. Flips any sites whose posterior is greater than or equal
        to altall_cutoff.

    infer_gaps : bool (default='true')
        If tree, uses Fitch's algorithm to infer gaps in the ancestral
        sequences.

    aaRatefile : str (default='lg')
        Evolutionary model to use for reconstruction. Read more about
        these models in the PAML documentation.
    ...
    df_ancs : phylopandas.DataFrame
        dataframe containing information (including sequences) about ancestors
        of the tree.
    ...
    n, m = len(df_seq), len(df_seq[sequence_col][0])
    alignment_file = 'alignment.phy'
    alignment_path = os.path.join(working_dir, alignment_file)
    alignment_str = df_seq.phylo.to_fasta(sequence_col=sequence_col, id_col=id_col,
                                          id_only=True)
    ...
    # Infer gaps in ancestral sequences?
    if infer_gaps:
